...
#define US_PER_OVERFLOW                 (512UL * NRF_802154_SL_US_PER_S) ///< Time that has passed between overflow events. On full RTC speed, it occurs every 512 s.
#define MIN_RTC_COMPARE_EVENT_DT        (2 * NRF_802154_SL_US_PER_TICK)  ///< Minimum time delta from now before RTC compare event is guaranteed to fire.
...
static void rtc_irq_handler(void)
{
    ...
    {
        // Clear event unconditionally
        nrf_rtc_event_clear(NRF_802154_RTC_INSTANCE, m_cmp_ch[SYNC_CHANNEL].event);

        // According to the RTC documentation, it might happen that:
        // 'If the COUNTER is N and the current CC register value is N+1 or N+2 when a new CC value
        // is written, a match may trigger on the previous CC value before the new value takes effect.`
        // Therefore, it is possible that RTC->EVENT_COMPARE fires for an already outdated value of
        // the CC register. If that's the case, the updated target time is larger than current time.
        // Ignore the interrupt if that happens.
        // Also, since both operands are uint64_t, there's no need to handle their overflow here.
        if (curr_time_get() >= m_target_times[SYNC_CHANNEL])
        {
            nrf_rtc_event_disable(NRF_802154_RTC_INSTANCE, m_cmp_ch[SYNC_CHANNEL].event_mask);
            nrf_rtc_int_disable(NRF_802154_RTC_INSTANCE, m_cmp_ch[SYNC_CHANNEL].int_mask);
